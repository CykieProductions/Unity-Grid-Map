...
namespace CykieProductions.General2D
{
    ...
    public class CameraFollow2D : MonoBehaviour
    {
        ...
        void SetTarget()
        {
            if (targetPlayer)
            {
                if (IGameManager.Main == null || IGameManager.Main.Player == null)
                {
                    ...
                    target = IGameManager.Main.Player;
            }
            ...
            if (curTarget == null)
            {
                Debug.LogWarning("Camera: \"" + gameObject.name + "\" does not have a target");
            }
            else
            {
        ...
        void Update()
        {
            ...
            if (Vector2.Distance(transform.position, curTarget.position) > 20)
            {
                transform.position = curTarget.position + offset;
            }
